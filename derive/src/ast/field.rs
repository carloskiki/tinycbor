...
impl Field {
    ...
    fn input<const LEN: bool>(&self) -> (TokenStream, TokenStream) {
        ...
        if let Some(tag) = self.tag {
            let source_ty = ty.clone();
            ty = quote! { ::tinycbor::tag::Tagged<#ty, #tag> };
            input = quote! { <&#ty as ::core::convert::From<&#source_ty>>::from(#input) };
        };
        (ty, input)
    }

    pub fn encode(self) -> TokenStream {
        let (ty, input) = self.input::<false>();
