...
- `U8` lifetime extension possible through a `From` implementation.
- Ensure streaming decoding of indefinite maps can't decode an odd number of items.
- Bound sizes provided to dynamic collections in `decode` to the size of the input slice.

# 0.12.5 - 2026-07-07

### Fixed

- `Cow` implementation of `Decode` when falling back to owned decoding.
- `Decode` implementation for `Encoded`, which previously allowed trailing bytes.
- Provides sizes to dynamic collections in `decode` when available.

# 0.12.4 - 2026-07-02

### Added

- `Decode` implementation for `Box<[(K, V)]>`.

# 0.12.3 - 2026-07-01

### Added

- `Decode` implementation for `Box<[T]>`.

# 0.12.2 - 2026-05-12

### Changed

- More flexible lifetime bounds on `With`'s `Decode` implementation.
...
# 0.12.0 (0.15.2-derive) - 2026-04-03

### Changed

- `Lazy::decode` returns `T` instead of `With<T>`.

# 0.11.3 - 2026-04-03

### Added

- `encoded` modules with wrapper types for maintaining the encoded form of a value.
